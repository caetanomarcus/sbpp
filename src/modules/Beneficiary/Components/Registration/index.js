...
const Registration = () => {
	...
	// const { cep, address, number, district, city, hasDifferentAddress } = addressDefault // hasMailAdress, correspondenceType
	...
	// const { product, susep, proposalNumber, proposalDate, salesStructure, taxationType } = benefitData
	...
	// const financialState = [national,accountType, bank, paymentMethod, accountNumber, accountDigit, agency, agencyDigit]
	...
	const enableButton = () => {
		switch (step) {
			...
			case benefit:
				return  true //canSend(benefitState) && canSend(paymentState) && canSend(agreementState, conditionalStates(agreementStates)) && canSend([true], conditionalStates(verifyStates(IRPFStates))) && canSend([true], conditionalStates(verifyStates(PSStates))) && canSend([true],conditionalStates(courtPensionStates))
			default:
				return false
		}
	}
